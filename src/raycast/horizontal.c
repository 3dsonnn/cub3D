...
{
	tmp->crd.y = floor(cub->player.pos.y0 / TILE_SIZE) * TILE_SIZE;
    if (FACE_DOWN(ray->angle))
        tmp->crd.y += TILE_SIZE;
    tmp->crd.x = cub->player.pos.x0 + ((tmp->crd.y - cub->player.pos.y0) / cub->rays[i].tan);
}

static  void    get_steps(t_cub *cub, t_intersection *tmp, int i)
{
	tmp->step.y = TILE_SIZE;
	if (FACE_UP(ray->angle))
		tmp->step.y *= -1;
	tmp->step.x = TILE_SIZE / cub->rays[i].tan;
...
static  inline  void    get_distance(t_cub *cub, int i)
{
    if (cub->rays[i].hor.intersected)
        cub->rays[i].hor.dist = sqrt(pow((cub->rays[i].hor.wall.x - cub->player.pos.x0), 2) + pow((cub->rays[i].hor.wall.y - cub->player.pos.y0), 2));
    else
