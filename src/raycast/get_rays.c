...
/*   Updated: 2025/02/13 15:20:51 by efinda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static  void    get_column(t_cub *cub, int i)
{
    ...
    cub->rays[i].col.height = (int)(TILE_SIZE / cub->rays[i].col.dist * cub->ppd);
...
void    get_rays(t_cub *cub, int i)
{
    ...
    while (++i < cub->img.width)
    {
        ...
        cub->rays[i].angle = normalize_angle(cub->rays[i].angle);
