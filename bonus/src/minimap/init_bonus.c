...
/*   Updated: 2025/05/06 16:40:36 by efinda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_tiles(t_cub *cub, t_tile ***tiles, int i, int j)
{
	*tiles = (t_tile **)malloc(sizeof(t_tile *) * (cub->scene.map.size.y));
	if (!(*tiles))
		exit_cub(cub, "Failed to allocate memory for the minimap tiles");
...
void	init_minimap_aux_imgs(t_cub *cub, t_mmap *mmap)
{
	...
	if (!mmap->circle.img)
		exit_cub(cub, "Failed to create the minimap circle image");
	if (!mmap->circle.addr)
		exit_cub(cub, "Failed to get the address of the minimap circle image");
	aux = (int)ceil(sqrt(2 * (mmap->circle.width * mmap->circle.width)));
	my_mlx_new_img(cub->mlx, &mmap->rotated, (t_point){aux, aux});
	if (!mmap->rotated.img)
		exit_cub(cub, "Failed to create the minimap rotated image");
	my_mlx_get_data_addr(&mmap->rotated);
	if (!mmap->rotated.addr)
		exit_cub(cub, "Failed to get the address of the minimap rotated image");
	my_mlx_duplicate_img(cub->mlx, &mmap->padded, mmap->rotated);
	if (!mmap->padded.img)
		exit_cub(cub, "Failed to create the minimap padded image");
	if (!mmap->padded.addr)
		exit_cub(cub, "Failed to get the address of the minimap padded image");
}
...
void	init_minimap(t_cub *cub)
{
	...
	if (!cub->minimap.img.img)
		exit_cub(cub, "Failed to create the minimap image");
	my_mlx_get_data_addr(&cub->minimap.img);
	if (!cub->minimap.img.addr)
		exit_cub(cub, "Failed to get the address of the minimap image");
